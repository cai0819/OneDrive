package com.qfedu.onedrive.admin.dao;
...
public interface MemberDao extends BaseMapper<Sys_member> {
    ...
    //查询用户的所有权限名称
    @Select("select p.prms from sys_permission p inner join sys_rolepermission rp on p.id=rp.pid inner join sys_userrole ur on ur.rid=rp.rid where p.type=1  and ur.uid=#{id}")
